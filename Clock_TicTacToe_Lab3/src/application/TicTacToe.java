...
public class TicTacToe extends Application {
	...
	public void start(Stage stage) throws Exception {
		...
		scene.getStylesheets().add(getResource("tictactoe-board.css"));
...
class GameControls extends HBox {
	...
	GameControls(final GameManager gameManager, final Game game) {
		...
		@Override
		public void handle(ActionEvent actionEvent) {gameManager.newGame();}});
		...
		@Override
		public void handle(ActionEvent actionEvent) {gameManager.quit();}});
...
class StatusIndicator extends HBox {
	...
	private void bindIndicatorFieldsToGame(Game game) {
		playerToken.imageProperty().bind(Bindings.when(game.currentPlayerProperty().isEqualTo(Square.State.NOUGHT)).then(SquareSkin.noughtImage).otherwise(Bindings.when(game.currentPlayerProperty().isEqualTo(Square.State.CROSS)).then(SquareSkin.crossImage).otherwise((Image) null)));
		playerLabel.textProperty().bind(Bindings.when(game.gameOverProperty().not()).then("Current Player: ").otherwise(Bindings.when(game.winnerProperty().isEqualTo(Square.State.EMPTY)).then("Draw").otherwise("Winning Player: ")));
	}
}
...
class SquareSkin extends StackPane {
	...
	static final Image noughtImage = new Image("https://pngimage.net/wp-content/uploads/2018/06/grunge-circle-png-4.png");
	static final Image crossImage = new Image("https://www.onlygfx.com/wp-content/uploads/2018/07/8-grunge-x-7-865x1024.png");
